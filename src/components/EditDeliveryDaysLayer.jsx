...
const AddDeliveryDaysLayer = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        axios
        .post(`${baseURL}/api/edit-delivery-days/${id}`, data, {
            headers: {
                'Authorization': `Bearer ${token}`,
            },
        })
    ...
    const fetchDeliveryDetails = async () => {
        try {
            const response = await fetch(`${baseURL}/api/get-delivery-day/${id}`, {
                headers: {
            ...
            if (data.status) {
                setSupplier(data.deliveryDays[0]?.supplier._id);
                setBranch(data.deliveryDays[0]?.branch._id);
                setSelectedAreas(data.deliveryDays[0]?.days);
            } else {
                alert('Failed to fetch delivery days details');
            }
        } catch (error) {
